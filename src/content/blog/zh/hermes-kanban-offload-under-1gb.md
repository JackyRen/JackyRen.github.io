...
最近我在整理自己的 agent 工作流，碰到一个很烦的问题：一个 agent 很容易被长任务拖住。改站点、跑浏览器测试、处理 OCR 文件、等 CI，哪一件都不一定难，但只要主线程挂在那里，内存、成本和注意力都会被一起占住。

后来我干脆把思路倒过来。中央 Hermes 不再亲自做所有重活，只做一个轻量的 boss：拆任务、盯状态、做验收。吃 CPU、IO、浏览器和文档处理资源的活，交给其他节点上的 worker team。
...
不到 1GB 不是炫技。我需要的是一个能长期在线的中央编排层。

如果中央 Hermes 只管任务图、卡片状态和少量决策，它就不该因为某个 Astro build、Playwright 浏览器或 OCR pipeline 而内存飙升。中央越薄，越适合常驻；worker 越独立，越适合按任务扩容、失败和重试。
...
我后来拿一份已经 hard-redacted 的历史数据库做了对比。这里不看原始聊天内容，也不引用私密文本，只看会话、消息、工具调用、时间戳和 continuation 这类元数据。样本包括 3 个 profile、2,607 个 session、47,292 条 message、20,217 次工具调用。

这份数据不能用来证明“短会话很多，所以人一定被打断很多”。有些 profile 里的极短 session 很可能只是 cron job 或系统自动化，应该从疲劳判断里剔除；parent session 也是 Hermes 架构的一部分，不代表人需要手动翻会话来恢复上下文。

最累的是权限 review churn。同一类命令反复被标成潜在危险，哪怕它们在当时的上下文里其实是安全的，也需要人一次次确认、放行、再确认。单次询问没什么，重复多了就会把注意力切碎。

sanitized usage export 只能给出聚合形态：全量样本里有 285 个 session 出现 3 次以上用户发言，690 次用户在 2 分钟内又接着回复 agent 或工具结果，257 个 continuation。内容已经被抹掉，所以我不会把每一次都解释成权限请求；但结合实际使用体验，它们至少提醒我，摩擦常常来自反复 review、反复确认和反复回到主对话，而不是某个 agent “不会干活”。
...
对个人项目来说，这已经够用了：不到 1GB 的中央编排层，加上可以按能力拆分的 offload 节点，就能让博客、前端 demo、OCR 和部署验证同时推进，不必都挤在一个永远 pending 的对话里。
